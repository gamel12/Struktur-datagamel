...
Queue adalah struktur data linear dengan prinsip **First In First Out (FIFO)** - elemen pertama masuk akan keluar pertama [1]. Aplikasi: penjadwalan proses, buffer management, breadth-first search [1].

Implementasi queue: array (ukuran tetap) atau linked list (ukuran dinamis) [2][3].

#### Struktur Queue

Queue memiliki dua pointer: **head** (depan untuk dequeue) dan **tail** (belakang untuk enqueue) [1].

Komponen queue [2]:
- **Data/Info**: Nilai elemen
- **Head/Front**: Pointer elemen depan
- **Tail/Rear**: Pointer elemen belakang
- **Capacity**: Kapasitas maksimum (array-based)

#### Operasi Dasar Queue

Operasi dasar C++ [1][2]:

1. **Enqueue**: Tambah elemen ke tail, O(1), cek overflow
2. **Dequeue**: Hapus elemen dari head, O(1), cek underflow
3. **Peek/Front**: Lihat elemen depan tanpa hapus, O(1)
4. **isEmpty**: Cek queue kosong (head == tail atau count == 0)
5. **isFull**: Cek queue penuh (count == MAX atau tail == MAX)
...
```
Efisien dengan operasi modulo untuk memutar indeks [2][3].
...
```
Fleksibel, ukuran dinamis, overhead memori untuk pointer [2][3].

#### Aplikasi Queue

Aplikasi [1][2]:
- **CPU Scheduling**: Round-robin, process management
- **Buffer Management**: Keyboard buffer, printer spooler
- **BFS**: Graph traversal
- **Interrupt Handling**: OS interrupt handling
- **Call Center**: Queue management
- **Async Transfer**: IO buffers, pipes

#### Keuntungan Queue

- Operasi O(1) cepat [1]
- Implementasi sederhana [2]
- Efisien untuk pola FIFO [1]
- Fairness & cocok untuk real-time [1]

#### Kekurangan Queue

- Tidak ada random access [1]
- Ukuran terbatas (array-based) [2]
- Pencarian tidak efisien [1]
- Circular queue butuh manajemen indeks hati-hati [2]

## Guided

### 1. GUIDED I - Queue dengan Linked List

Implementasi Queue dengan Linked List: operasi enqueue, dequeue, update, view, search.
